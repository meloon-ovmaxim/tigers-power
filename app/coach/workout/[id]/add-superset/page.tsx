...
export default function AddSuperset() {
  ...
  function addExercise(ex: any) {
    setItems(prev => [...prev, { exercise_id: ex.id, exercise_name: ex.name, reps: '12', weight_kg: '' }])
    setSearch(''); setShowSearch(false); setShowSuggestions(false); setShowNewEx(false)
  }
  ...
  async function createNewExercise() {
    ...
    const { data } = await supabase.from('exercises')
      ...
      .select().single()
    if (data) { setExercises(prev => [...prev, data]); addExercise(data); setNewExName('') }
  }
  ...
  async function handleSave() {
    ...
    const roundsNum = parseInt(rounds) || 3

    const { data: block } = await supabase.from('workout_blocks').insert({
      day_id: workoutId, type: 'superset', position: pos,
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      // One set row per round — подходы = круги
      await supabase.from('sets').insert(
        Array.from({ length: roundsNum }, (_, si) => ({
          be_id: be.id,
          set_number: si + 1,
    ...
    <div className="page">
      ...
      <div className="content">
        <div className="info-box">
          Упражнения выполняются подряд, затем отдых. Количество повторений — для каждого круга.
        </div>
        ...
        {items.map((item, idx) => (
          <div key={idx} style={{
            ...
            {/* Only reps + weight — no sets field */}
            <div className="grid-2">
              <div>
                <label className="label">Повторений за круг</label>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {showSuggestions && search.length > 0 && filtered.length > 0 && (
              <div className="suggest-list">
                {filtered.map(e => (
                  <div key={e.id} className="suggest-item" onMouseDown={() => addExercise(e)}>
                    {e.name}
                  </div>
                ))}
              </div>
            ...
            {search.length > 0 && !showNewEx && (
              <button className="btn btn-sm" style={{ marginTop: 6, color: 'var(--accent2)', borderColor: 'var(--accentbdr)' }}
            ...
            <button className="btn btn-ghost btn-sm" style={{ marginTop: 8 }}
              onClick={() => { setShowSearch(false); setSearch(''); setShowSuggestions(false) }}>
              Отмена
            </button>
          </div>
        ) : (
        ...
        <div className="form-group">
          ...
          <input className="input" placeholder="3" value={rounds} onChange={e => setRounds(e.target.value)} />
          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 4 }}>
            Каждый круг = все упражнения серии по одному разу
          </div>
        </div>
